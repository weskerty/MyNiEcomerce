...
export async function onRequest(context){
  ...
  const init={method:request.method,headers:{'x-bridge-key':env.BRIDGE_KEY,'Content-Type':'application/json','cf-connecting-ip':ip}};
  if(request.method!=='GET')init.body=await request.text();
  const res=await fetch(`${env.SERVER_URL}/productos${qs}`,init);
