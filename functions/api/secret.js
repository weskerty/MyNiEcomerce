...
export async function onRequestGet(context) {
  ...
  const target = id
    ? `${env.SERVER_URL}/SB/post?id=${encodeURIComponent(id)}&coff=${encodeURIComponent(u.searchParams.get('coff') || '0')}`
    : `${env.SERVER_URL}/SB/posts?off=${encodeURIComponent(u.searchParams.get('off') || '0')}`;
...
export async function onRequestPost(context) {
  ...
  const target = `${env.SERVER_URL}/SB/${isC ? 'comment' : 'post'}`;
