...
const FF_JS='https://unpkg.com/@ffmpeg/ffmpeg@0.12.15/dist/umd/ffmpeg.min.js';
const FF_W='https://unpkg.com/@ffmpeg/ffmpeg@0.12.15/dist/umd/814.ffmpeg.js';
const FF_CORE='https://unpkg.com/@ffmpeg/core@0.12.10/dist/umd/ffmpeg-core.js';
const FF_WASM='https://unpkg.com/@ffmpeg/core@0.12.10/dist/umd/ffmpeg-core.wasm';
...
async function bU(url,mime){
  const r=await fetch(url);
  if(!r.ok)throw new Error('fetch: '+url);
  const d=mime==='application/wasm'?await r.arrayBuffer():await r.text();
  return URL.createObjectURL(new Blob([d],{type:mime}));
}

A.onclick=()=>B.click();
A.ondragover=a=>{a.preventDefault();A.classList.add('vc-over');};
A.ondragleave=()=>A.classList.remove('vc-over');
A.ondrop=a=>{a.preventDefault();A.classList.remove('vc-over');const b=a.dataTransfer.files[0];if(b)T(b);};
B.onchange=a=>{const b=a.target.files[0];if(b)T(b);};
H.onclick=S;
...
async function V(){
  ...
  U=(async()=>{
    ...
    await new Promise((a,b)=>{
      ...
      c.src=FF_JS;
      c.onload=a;
      c.onerror=()=>b(new Error('ffmpeg.js fail'));
      document.head.appendChild(c);
    });

    if(!window.FFmpegWASM)throw new Error('FFmpegWASM missing');
    const {FFmpeg}=window.FFmpegWASM;
    const d=new FFmpeg();

    P(20,'Descargando assets (~31MB)...');
    const [wURL,cURL,waURL]=await Promise.all([
      bU(FF_W,'text/javascript'),
      bU(FF_CORE,'text/javascript'),
      bU(FF_WASM,'application/wasm'),
    ]);
...
async function W(a,b,c){
  ...
  const e='i_'+ts+'.mp4';
  const f='o_'+ts+'.mp4';
  await d.writeFile(e,new Uint8Array(await a.arrayBuffer()));
  await d.exec(['-ss',String(b),'-to',String(c),'-i',e,'-c','copy',f]);
  const g=await d.readFile(f);
  await d.deleteFile(e);
  await d.deleteFile(f);
  return new Blob([g.buffer],{type:'video/mp4'});
}
...
async function X(a){
  ...
  try{
    ...
    for(let c=0;c<b;c++){
      ...
      k.className='vc-dl';k.href=g;k.download=h;k.textContent='⬇ Descargar';
