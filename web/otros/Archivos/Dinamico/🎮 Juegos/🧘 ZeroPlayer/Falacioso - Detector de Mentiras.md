...
.FL_op{flex:1;padding:14px;border-radius:12px;border:1px solid rgba(255,255,255,.2);background:rgba(255,255,255,.05);cursor:pointer;text-align:center;transition:background .2s}
.FL_op:hover{background:rgba(255,255,255,.12)}
.FL_op.FL_dis{pointer-events:none;opacity:.7}
#FL_final{text-align:center;display:flex;flex-direction:column;gap:12px}
...
var FL_TOTAL=20;
var FL_TIME=15;
var FL_data=[],FL_orden=[],FL_idx=0,FL_ok=0,FL_fail=0,FL_int=null,FL_seg=0,FL_activo=false;
...
function FL_responder(resp,correcta){
  ...
  FL_activo=false;
  clearInterval(FL_int);
  if(resp===correcta)FL_ok++;else FL_fail++;
  document.querySelectorAll("#FL_opciones .FL_op").forEach(function(el){el.classList.add("FL_dis")});
  FL_idx++;
...
fetch(FL_base+"falacias.json")
  ...
  .catch(function(e){
    document.getElementById("FL_wrap").textContent="Error cargando el juego "+e;
  });
